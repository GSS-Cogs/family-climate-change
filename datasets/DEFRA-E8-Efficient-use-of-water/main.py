...
df['Marker'] = df.apply (lambda x: 'not-available' if 'not-available' in x['Value'] else '', axis = 1)
df['Value'] = df.apply (lambda x: '' if 'not-available' in x['Value'] else x['Value'], axis = 1)
