...
for tab in tabs:
    ...
    # remove_bottom_section = tab.filter('SIC(07 group').expand(DOWN).expand(RIGHT)
    year = tab.excel_ref('D4').expand(RIGHT).is_not_blank()
    section = tab.excel_ref('A5').expand(DOWN)
    section_name = tab.excel_ref('C5').expand(DOWN)
    ...
    observations = year.fill(DOWN).is_not_blank() - \
        remove_top_section - remove_notes
