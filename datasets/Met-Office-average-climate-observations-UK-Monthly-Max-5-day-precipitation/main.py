...
df.to_csv(out/'max-5-day.csv', index = False)
...
csvw_mapping.set_csv(out/"max-5-day.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'max-5-day.csv-metadata.json')

shutil.copy("max-5-day.csv-metadata.trig", out/"max-5-day.csv-metadata.trig")
