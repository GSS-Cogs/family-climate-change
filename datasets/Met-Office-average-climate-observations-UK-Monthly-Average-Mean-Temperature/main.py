...
df.to_csv(out/'mean-temp.csv', index = False)
...
csvw_mapping.set_csv(out/"mean-temp.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'mean-temp.csv-metadata.json')

shutil.copy("mean-temp.csv-metadata.trig", out/"mean-temp.csv-metadata.trig")
