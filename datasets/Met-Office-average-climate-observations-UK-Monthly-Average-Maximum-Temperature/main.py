...
df.to_csv(out/'max-temp.csv', index = False)
...
csvw_mapping.set_csv(out/"max-temp.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'max-temp.csv-metadata.json')

shutil.copy("max-temp.csv-metadata.trig", out/"max-temp.csv-metadata.trig")
