...
df.to_csv(out/'monthly-avg-max-temp.csv', index = False)
...
csvw_mapping.set_csv(out/"monthly-avg-max-temp.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'monthly-avg-max-temp.csv-metadata.json')

shutil.copy("monthly-avg-max-temp.csv-metadata.trig", out/"monthly-avg-max-temp.csv-metadata.trig")
