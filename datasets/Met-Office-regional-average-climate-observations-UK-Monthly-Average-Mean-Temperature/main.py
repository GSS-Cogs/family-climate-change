...
df.to_csv(out/'monthly-avg-mean-temp.csv', index = False)
...
csvw_mapping.set_csv(out/"monthly-avg-mean-temp.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'monthly-avg-mean-temp.csv-metadata.json')

shutil.copy("monthly-avg-mean-temp.csv-metadata.trig", out/"monthly-avg-mean-temp.csv-metadata.trig")
