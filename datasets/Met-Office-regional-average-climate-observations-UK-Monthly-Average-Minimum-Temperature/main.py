...
df.to_csv(out/'monthly-avg-min-temp.csv', index = False)
...
csvw_mapping.set_csv(out/"monthly-avg-min-temp.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'monthly-avg-min-temp.csv-metadata.json')

shutil.copy("monthly-avg-min-temp.csv-metadata.trig", out/"monthly-avg-min-temp.csv-metadata.trig")
