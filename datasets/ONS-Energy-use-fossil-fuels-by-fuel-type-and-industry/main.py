...
for tab in tabs:
    ...
    remove = (
        tab.filter("Notes").expand(DOWN).expand(RIGHT)
        | tab.filter("SIC (07) group")
        | tab.filter("Section")
    )
    year = pivot.shift(3, 0).expand(RIGHT).is_not_blank()
    sicSection = tab.filter("Section").expand(DOWN) - remove | tab.excel_ref(
        "A5"
    ).expand(DOWN) - remove.expand(DOWN)
    sicGroup = tab.filter("SIC (07) group").expand(DOWN) - remove | tab.excel_ref(
        "B5"
    ).expand(DOWN) - remove.expand(DOWN)
    industry = pivot.shift(2, 0).expand(DOWN).is_not_blank() - remove

    if tab.name != "Aviation fuel":
        ...
        fuel = (
            tab.excel_ref("D6").expand(DOWN).is_not_blank()
            - tab.excel_ref("D25").expand(DOWN).is_not_blank()
        )
...
df.replace(
    {
        ...
        "Fuel": {"('DERV',)": "DERV", "Carbon based fuels ": "Carbon based fuels"},
    },
    inplace=True,
)
...
df.replace(
    {
        "Fuel": {"DERV": "Diesel oil for road vehicles (DERV)"},
    },
    inplace=True,
)
...
df["SIC Group"] = df.apply(
    lambda x: x["SIC Section"]
    if x["SIC Group"] == "" or x["SIC Group"] == "-"
    else x["SIC Group"],
    axis=1,
)
df["SIC Group"] = df.apply(
    lambda x: x["Industry"]
    if x["SIC Group"] == "-" and x["SIC Section"] == "-"
    else x["SIC Group"],
    axis=1,
)
df["SIC Group"] = df.apply(
...
COLUMNS_TO_NOT_PATHIFY = ["Year", "Section", "Value", "Marker"]
