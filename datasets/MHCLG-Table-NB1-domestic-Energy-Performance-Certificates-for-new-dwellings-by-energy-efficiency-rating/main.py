...
class DateProcessing:
    ...
    def date_format(f):
        def date_wrapper(self):
            self.data_frame["Period"] = self.data_frame["Period"].astype(str).apply(lambda x: "year/" + x[:4] if len(x) == 4 else "quarter/" + x[:4] + "-0" + x[5:6] if len(x) == 6 else '')
            df = self.data_frame[['Period', 'Lodgements', 'Total Floor Area (m2)', 'Location',
                            'Efficieny Rating', 'Value', 'Measure Type', 'Unit']]
            return df
        return date_wrapper
...
if __name__ == "__main__":
    ...
    final_df = DateProcessing(data_frame=df)
    print(final_df.date_process().head(5))