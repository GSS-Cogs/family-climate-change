...
df.to_csv(out/'air-frost.csv', index = False)
...
csvw_mapping.set_csv(out/"air-frost.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'air-frost.csv-metadata.json')

shutil.copy("air-frost.csv-metadata.trig", out/"air-frost.csv-metadata.trig")
