...
df.to_csv(out/'rainfall-intensity.csv', index = False)
...
csvw_mapping.set_csv(out/"rainfall-intensity.csv")
csvw_mapping.set_dataset_uri(f"http://gss-data.org.uk/data/gss_data/climate-change/{info_json['id']}")
csvw_mapping.write(out/'rainfall-intensity.csv-metadata.json')

shutil.copy("rainfall-intensity.csv-metadata.trig", out/"rainfall-intensity.csv-metadata.trig")
